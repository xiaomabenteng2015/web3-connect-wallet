...
export const useBalances = ({ address }: { address: `0x${string}` }) => {
  ...
  // 查询USDT代币余额
  const { data: USDTBalance, queryKey: USDTBalanceQueryKey } =
    useReadUSDTBalanceOf({
      args: [address],
    });

  // 查询地址对交易所的USDT授权额度
  const { data: USDTAllowance, queryKey: USDTAllowanceQueryKey } =
    useReadUSDTAllowance({
      args: [address, USDT_CONTRACT_ADDRESS],
    });
